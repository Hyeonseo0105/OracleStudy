...
public class StudentDAO {
	...
	public List<StudentVO> stdListData()
	{
		...
		try
		{
			...
			// Back-End(SQL) , Front-End(JSON(자바스크립트의 VO,객체표현법)/JSONP)
			// VO => {} , List => [] => [{},{},{}...]
			// XML => JSON ==> Aj(자바스크립트)a(and)x(XML)
	...
	public void  stdInsert(StudentVO vo)
	{
		try
		{
			...
			// 실행
			ps.executeUpdate();  //데이터베이스가 변동 => executeUpdate()
								 //COMMIT을 가지고 있음
		}catch(Exception ex)
		{
			ex.printStackTrace();
	...
	// 3. 상세보기 => SELECT WHERE
	 ...
	 public void stdUpdate(StudentVO vo)
	 {
	  ...
	  // 5. 삭제 => DELETE => COMMIT	 public void stdDelete(int hakbun)
